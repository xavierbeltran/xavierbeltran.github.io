...
// Variable for model view and current transformation matrix
var ctm;
var modelViewMatrixLoc;

// Variable to hold the rotation and direction of the "mouse"
var circle_rotation = 0;
var circle_direction = 0;
...
window.onload = function init() {
    ...
    // Function to draw the circle at a center point, cirX and cirY with a radius of 1/30
    // Note that it draws two circles to draw the "mouse"
    function drawCircle(cirX = 0, cirY = 0, radius = 1 / 30) {
        let theta_val = 0;
        ...
        theta_val = 0;
        for (let i = 0; i < circle_val; i++) {
            vertices.push(vec2(radius * (Math.cos(theta_val)) + cirX - 1 / 20, radius * (Math.sin(theta_val)) + cirY));
            colors.push(vec3(1, 0, 0));
            theta_val = theta_val + pi_val;
        }
    }
    ...
    window.onkeydown = function (event) {
        // If at the end of the maze, do not allow the user to move
        if (cirEnd) {
            return;
        }
        switch (event.key) {
            // If the user presses the right arrow key, rotate the circle clockwise
            case "ArrowRight":
                circle_rotation += 90;
                circle_direction += 1;
                break;
            // If the user presses the left arrow key, rotate the circle counter-clockwise
            case "ArrowLeft":
                circle_rotation -= 90;
                circle_direction -= 1;
                break;
            // If the user presses the up arrow key, move the circle forward
            case "ArrowUp":
                // Get the direction the circle is facing
                var dir = circle_direction % 4;
                // If the direction is -0, set it to 0
                if (dir == -0) dir += 0;
                // If the direction is negative, add 4 to it to get the right location
                if (dir < 0) dir += 4;
                // Check if the move is valid
                if (isValidMove(circleX, circleY, dir)) {
                    // If so, translate the circle to the right spot
                    switch (dir) {
                        case 0:
                            circleX -= cell_iter;
                            break;
...
function render() {
    ...
    if (cirEnd) {
        ...
        // Rotates the point and then translates it to the correct position
        ctm = mat4();
        ctm = mult(ctm, translate(circleX, circleY, 0));
        ctm = mult(ctm, rotateZ(-circle_rotation));
    }

    gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(ctm));
    gl.drawArrays(gl.TRIANGLE_FAN, buffer_val += line_val, circle_val);
    gl.drawArrays(gl.TRIANGLE_FAN, buffer_val += circle_val, circle_val);
